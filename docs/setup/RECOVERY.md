...
```bash
# 如需系統開機自啟
cp ~/projects/axc-trading/launchagents/*.plist ~/Library/LaunchAgents/
...
(crontab -l 2>/dev/null; \
 echo "0 3 * * * bash ~/projects/axc-trading/scripts/backup_agent.sh >> ~/projects/axc-trading/logs/backup.log 2>&1") \
 | crontab -
```
