...
def render_notification_bell():
    ...
    with ui.button(icon='notifications', on_click=lambda: panel.set_visibility(not panel.visible)) \
            .props('flat round color=white size=sm').style('position:relative;'):
        badge.move(target_container=ui.context.client.layout)  # won't work inline, use JS
    ...
    with panel:
        with ui.row().classes('items-center justify-between px-3 py-2 border-b border-gray-800'):
            ...
            ui.button('Clear', on_click=lambda: _clear_all(notif_container, badge)) \
                .props('flat dense size=xs color=grey-6')
        notif_container

    def _clear_all(container, bdg):
        app.storage.user['notifications'] = []
        app.storage.user['notif_last_read'] = time.time()
        container.clear()
        bdg.text = '0'
        bdg.set_visibility(False)
