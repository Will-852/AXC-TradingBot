...
PipelineContext Protocol: 任何有 verbose/errors/warnings field 嘅 context 都可以用。
CycleContext 同 PolyContext 都自然滿足。
"""
...
class PipelineContext(Protocol):
    """Minimal context interface for pipeline execution."""
    verbose: bool
    errors: list[str]
    warnings: list[str]
...
class Pipeline:
    """
    Ordered sequence of steps. Each step takes context and returns it.
